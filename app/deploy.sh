...
run_gcloud run deploy "${SERVICE}" \
  ...
  --update-env-vars "GOOGLE_CLOUD_PROJECT=${PROJECT_ID},REGION=${REGION},SIM_MODE=${SIM_MODE},REPLAY_MODE=${REPLAY_MODE},PUBLIC_PROJECT=${PUBLIC_PROJECT},BETA_DEVELOPER_KEY_TTL_HOURS=168,LIVE_CALL_DAILY_CAP=60,LIVE_CALL_PER_CALLER_CAP=12" \
  "${BETA_SECRET_ARGS[@]}" \
